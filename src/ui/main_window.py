...
class Worker(QThread):
    ...
    progress = pyqtSignal(int)          # thread-safe progress
    ...
    def run(self):
        try:
            # Pass self.progress.emit as the progress callback
            # so subprocesses can safely update the UI via Qt signal
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Poll server status every 3s -- no subprocess, no flicker
        self._timer = QTimer(self)
        self._timer.timeout.connect(self._poll)
        self._timer.start(3000)

        # Auto-load databases shortly after startup if server already running
        ...
        # Start mDNS immediately on launch — always broadcasting pgops.local
        QTimer.singleShot(500, self._auto_start_mdns)

    def _initial_load(self):
        """Called once on startup -- loads databases if server is already running."""
        self._poll()
    ...
    def _tabs(self):
        ...
        # Start activity monitor when its tab is selected
    ...
    def _generate_cert(self):
        ...
        def fn(_prog):
            return generate_certificate(BASE_DIR)
        def done(ok, msg):
    ...
    def _enable_ssl(self):
        if not is_ssl_configured(BASE_DIR):
            QMessageBox.warning(self, "No Certificate",
                "Generate a certificate first.")
            return
        if not self.manager.is_initialized():
            QMessageBox.warning(self, "Not Initialized",
                "Start the server at least once first.")
            return
        def fn(_prog):
            return enable_ssl(BASE_DIR, DATA_DIR)
        def done(ok, msg):
            self._log(msg)
            self._update_ssl_status()
            if ok:
                QMessageBox.information(self, "SSL Enabled",
    ...
    def _tab_server(self):
        ...
        for b in (self.btn_pgadmin_start, self.btn_pgadmin_stop, self.btn_pgadmin_open):
    ...
    def _tab_backup(self):
        ...
        # Progress bar -- updated via Qt signal, never directly from worker thread
    ...
    def _tab_settings(self):
        ...
        pv = pw_box.layout()
        pv.addWidget(_lbl(
            "Change the master password used to unlock PGOps on launch.",
            "#64748b", 11
        ))
    ...
    def _tab_network(self):
        # Outer widget -- tab receives this
        outer = QWidget()
        outer_layout = QVBoxLayout(outer)
        outer_layout.setContentsMargins(0, 0, 0, 0)
    ...
    def _on_start_done(self, ok, msg):
        ...
        if ok:
            ...
            self._update_ssl_status()
            # Auto-start MinIO alongside PostgreSQL
            ...
            # Auto-start pgAdmin alongside PostgreSQL
    ...
    # ── Database loading -- async, never blocks UI ─────────────────────────────
    def _load_databases_async(self):
        """Load database list in a worker thread -- called on startup and after operations."""
        if not self.manager.is_running():
            return
        cfg = self.config
        def fn(_prog):
            dbs = dbm.list_databases(cfg["username"], cfg["password"], cfg["port"])
            return True, dbs   # pass dbs as the msg slot (hack: reuse tuple)

        def done(ok, raw):
            # raw is the string repr if error, or we need to re-fetch
            # Since Worker stringifies msg, re-fetch directly on success
            if ok:
                try:
                    dbs = dbm.list_databases(cfg["username"], cfg["password"], cfg["port"])
    ...
    def _do_backup(self):
        ...
        # Capture config values now -- don't access self.config from worker thread
        user = self.config["username"]
        pw   = self.config["password"]
        port = self.config["port"]

        def fn(prog_cb):
            # prog_cb is self.progress.emit -- safe Qt signal, crosses threads fine
        ...
        w = self._run(fn, done)
        w.progress.connect(self.bk_prog.setValue)   # safe -- Qt signal queued connection
    ...
    def _update_ssl_status(self):
        ...
        if cert_info and "expires" in cert_info:
            self.ssl_cert_info_lbl.setText(
                f"Certificate: {cert_info.get('subject','--')}  |  "
    ...
    def _enable_ssl(self):
        ...
        def done(ok, msg):
            ...
            self._update_ssl_status()
            if ok:
                QMessageBox.information(self, "SSL Enabled", msg + " Restart the server to apply.")
            else:
                QMessageBox.critical(self, "Error", msg)
        self._run(fn, done)
    ...
    def _update_pgadmin_status(self):
        if not hasattr(self, "pgadmin_status_lbl"):
            return
        if not self.pgadmin.is_available():
            self.pgadmin_status_lbl.setText(
                "Not available -- Setup PostgreSQL first"
            )
            self.pgadmin_status_lbl.setStyleSheet(
                "color:#475569;font-size:13px;font-weight:bold;"
            )
            return
        if self.pgadmin.is_running():
            self.pgadmin_status_lbl.setText(
                f"RUNNING  --  {self.pgadmin.url()}"
            )
            self.pgadmin_status_lbl.setStyleSheet(
    ...
    def _auto_start_mdns(self):
        """Called once on launch — starts mDNS and keeps it running always."""
    ...
    def _update_mdns_status(self):
        ...
        if self.mdns.is_running():
            ip = self.manager.get_lan_ip()
            self.mdns_status_lbl.setText(
                "BROADCASTING  pgops.local  ->  " + ip
            )
            ...
            self.mdns_status_lbl.setText(
                "Not broadcasting -- click Start Broadcasting to resume"
            )
            self.mdns_status_lbl.setStyleSheet("color:#ef4444;font-size:13px;font-weight:bold;")

    def _refresh_interfaces(self):
        """Populate the interface table with all available network IPs."""
        try:
    ...
    def _pin_selected_ip(self):
        ...
        self._log(f"Pinned host IP: {ip} -- all apps should use this address.")
        self._poll()  # refresh connection string display
    ...
    def _run(self, fn, on_done) -> Worker:
        """Spawn a worker. fn(progress_callback) -> (ok, msg). Returns the worker."""
        w = Worker(fn)
        w.done.connect(on_done)
        w.start()
        self._workers.append(w)
        self._workers = [x for x in self._workers if x.isRunning()]
        return w   # caller can connect w.progress if needed
